...
class BestPosStrategy:
    ...
    def __init__(self, delay: float, sigma: float, gamma: float, k: float, hold_time:Optional[float]) -> None:
        ...
        self.T_begin = 1655942402250125991 #beginning of our session
        self.T_end = 1655943545128514890 #end of our session
    ...
    def run(self, sim: Sim ) ->\
        ...
        while True:
            ...
            if receive_ts - prev_time >= self.delay:
                ...
                ask_price = mid + 0.5 * (spread - mid_reservation_spread)
                bid_price = mid - 0.5 * (spread + mid_reservation_spread)
